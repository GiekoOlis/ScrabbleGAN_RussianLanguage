    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if opt.num_critic_train == 1:
                ...
                # print('\n\nG',model.loss_G, '\nD', model.loss_D, '\nDreal',model.loss_Dreal, '\nDfake', model.loss_Dfake,
                #       '\nOCR_real', model.loss_OCR_real, '\nOCR_fake', model.loss_OCR_fake, '\ngrad_fake_OCR', model.loss_grad_fake_OCR, '\ngrad_fake_adv', model.loss_grad_fake_adv)
